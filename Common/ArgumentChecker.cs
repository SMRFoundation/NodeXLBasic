...
internal class ArgumentChecker : Object
{
    ...
    (
        String sPropertyName,
        Double dPropertyValue,
        Double dMinimumValidValue,
        Double dMaximumValidValue
    )
    {
        AssertValid();
        Debug.Assert(sPropertyName != null);
        Debug.Assert(sPropertyName.Length > 0);
        Debug.Assert(dMaximumValidValue >= dMinimumValidValue);

        if (dPropertyValue < dMinimumValidValue ||
            dPropertyValue > dMaximumValidValue)
        {
            ThrowPropertyException(sPropertyName, String.Format(
                
                "Must be between {0} and {1}."
                ,
                dMinimumValidValue, dMaximumValidValue
                ) );
        }
    }
    ...
    )
    {
        ...
        if (dArgumentValue < dMinimumValidValue ||
            dArgumentValue > dMaximumValidValue)
        {
            String sMessageDetails = String.Format(

                "{0} argument must be between {1} and {2}."
                ,
                sArgumentName,
                dMinimumValidValue,
                dMaximumValidValue
