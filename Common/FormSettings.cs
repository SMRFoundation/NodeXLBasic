...
public class FormSettings : ApplicationSettingsBase
{
    ...
    (
        Form form,
        Boolean formIsFixedSize
    )
    {
        Debug.Assert(form != null);

        // Note: Although it would be possible to determine whether the form is
        // of fixed size by looking at the form's FormBorderStyle property,
        // that would require that the form's Initialize() method be called
        // before this constructor is called.  It's more reliable to require a
        // constructor parameter for this.

        m_bFormIsFixedSize = formIsFixedSize;
    ...
    protected Boolean m_bFormIsFixedSize;
}
