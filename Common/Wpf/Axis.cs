...
public class Axis : FrameworkElement
{
    ...
    )
    {
        ...
        m_bValuesAreLogarithmic = valuesAreLogarithmic;
    ...
    )
    {
        ...
        {
            // When this control is used in the NodeXLWithAxesControl, which
            // initially hides its axes, OnRender() gets called several times
            // at startup with a DesiredSize width or height of 0.  When the
            // code below was executed in that case, a mysterious assertion (at
            // Debug.Assert(dAdjustedNearOffset >= 0)) occurred in the other
            // OnRender() method.  That assertion was seemingly impossible, so
            // this workaround isn't really satisfactory.
        ...
        Double dY1 = GetLogIfRequired(dNearValue);
        Double dX2 = dAdjustedFarX;
        Double dY2 = GetLogIfRequired(dFarValue);
        ...
        if (dAdjustedNearOffset < 0)
        {
            ...
            dAdjustedNearValue = GetPowIfRequired( dY1 + dM * (0 - dX1) );
        ...
        else
        {
            ...
            dAdjustedFarValue =
                GetPowIfRequired(dY1 + dM * (dAxisLength - dX1) );
    ...
    {
        // These were used for tracking down a mysterious bug in which
        // dAdjustedFarOffset was negative.  See the comments in the OnRender()
        // method that called this one.

        Debug.Assert(oDrawingContext != null, "Null oDrawingContext.");

        Debug.Assert(dAdjustedNearOffset >= 0,
            "Negative dAdjustedNearOffset 3.");

        Debug.Assert(dAdjustedFarOffset >= 0,
            "Negative dAdjustedFarOffset 3.");
        ...
        if (m_bValuesAreLogarithmic)
        {
            adMajorGridlineValues = ChartUtil.GetLogAxisGridlineValues(
                dAdjustedNearValue, dAdjustedFarValue);
        ...
        else
        {
            ...
            ChartUtil.GetAxisGridlineValues(dAdjustedNearValue,
                dAdjustedFarValue, out adMajorGridlineValues,
                out iDecimalPlacesToShow);

            sFormatSpecifier = GetFormatSpecifier(adMajorGridlineValues,
        ...
        Double dX1 = GetLogIfRequired(dAdjustedNearValue);
        Double dY1 = dAdjustedNearOffset;
        Double dX2 = GetLogIfRequired(dAdjustedFarValue);
        ...
        foreach (Double dMajorGridlineValue in adMajorGridlineValues)
        {
            ...
            Double dMajorGridlineOffset =
                dY1 + dM * ( GetLogIfRequired(dMajorGridlineValue) - dX1);
    ...
    )
    {
        ...
        Double [] adMinorGridlineOffsets;

        if (m_bValuesAreLogarithmic)
        {
            adMinorGridlineOffsets = new Double[8];

            for (Int32 i = 2; i <= 9; i++)
            {
                adMinorGridlineOffsets[i - 2] =
                    dMajorGridline1Offset +
                    Math.Log10(i) *
                        (dMajorGridline2Offset - dMajorGridline1Offset);
            }
        }
        else
        {
            adMinorGridlineOffsets = new Double[MinorGridlines];
            ...
            for (Int32 i = 0; i < MinorGridlines; i++)
            {
                ...
                adMinorGridlineOffsets[i] = dMinorGridlineOffset;
            }
        }

        foreach (Double dMinorGridlineOffset in adMinorGridlineOffsets)
        {
            DrawGridline(oDrawingContext, oClientRectangle, oPen,
    ...
    protected Boolean m_bValuesAreLogarithmic;
