...
///
/// <para>
/// Set the <see cref="PutNeighborlessVerticesInOneCluster" /> property to true
/// to put the graph's neighborless vertices into one cluster.
/// </para>
///
/// <para>
/// IMPORTANT NOTE: If <see cref="PutNeighborlessVerticesInOneCluster" /> is
/// true, <see cref="CalculateGraphMetrics" /> removes any neighborless
/// vertices from the graph that is passed to it.
/// </para>
///
/// </remarks>
//*****************************************************************************
...
public class ClusterCalculator : GraphMetricCalculatorBase
{
    ...
    public ClusterCalculator()
    {
        ...
        m_bPutNeighborlessVerticesInOneCluster = false;
    ...
    PutNeighborlessVerticesInOneCluster
    {
        get
        {
            ...
            return (m_bPutNeighborlessVerticesInOneCluster);
        }

        set
        {
            m_bPutNeighborlessVerticesInOneCluster = value;
    ...
    /// edges and self-loops.  See the IMPORTANT NOTE in the remarks.
    /// </param>
    ///
    /// <returns>
    ...
    ///
    /// <remarks>
    /// IMPORTANT NOTE: If <see cref="PutNeighborlessVerticesInOneCluster" />
    /// is true, this method removes any neighborless vertices from <paramref
    /// name="graph" />.  It does not bother to add them back afterward.
    /// </remarks>
    //*************************************************************************
    ...
    )
    {
        ...
        graphMetrics = null;
        List<IVertex> oNeighborlessVertices = null;

        if (m_bPutNeighborlessVerticesInOneCluster)
        {
            oNeighborlessVertices = RemoveNeighborlessVerticesFromGraph(graph);
        }

        Boolean bGraphMetricsCalculated;

        switch (m_eAlgorithm)
        {
            case ClusterAlgorithm.WakitaTsurumi:

                bGraphMetricsCalculated = TryCalculateClustersWakitaTsurumi(
                    graph, backgroundWorker, out graphMetrics);

                break;

            case ClusterAlgorithm.GirvanNewman:

                bGraphMetricsCalculated = TryCalculateClustersSnap(graph,
            ...
            case ClusterAlgorithm.Clique:

                bGraphMetricsCalculated = TryCalculateClustersSnap(graph,
                    SnapGraphMetrics.Cliques,
                    backgroundWorker, out graphMetrics);
        ...
        if (
            bGraphMetricsCalculated
            &&
            m_bPutNeighborlessVerticesInOneCluster
            &&
            oNeighborlessVertices.Count > 0
            )
        {
            graphMetrics.Add(
                CreateCommunityOfNeighborlessVertices(oNeighborlessVertices) );
        }

        return (bGraphMetricsCalculated);
    }

    //*************************************************************************
    //  Method: RemoveNeighborlessVerticesFromGraph()
    //
    /// <summary>
    /// Removes any neighborless vertices from the graph.
    /// </summary>
    ///
    /// <param name="oGraph">
    ...
    ///
    /// <returns>
    /// A collection of the neighborless vertices that were removed.
    /// </returns>
    //*************************************************************************

    protected List<IVertex>
    ...
    //*************************************************************************
    //  Method: CreateCommunityOfNeighborlessVertices()
    //
    /// <summary>
    /// Creates a Community object that contains a specified collection of
    /// neighborless vertices.
    /// </summary>
    ///
    /// <param name="oNeighborlessVertices">
    /// The neighborless vertices to include in the community.
    /// </param>
    ///
    /// <returns>
    /// A new Community object that contains <paramref
    /// name="oNeighborlessVertices" />.
    /// </returns>
    //*************************************************************************
    ...
    /// true to put the neighborless vertices into one cluster.

    protected Boolean m_bPutNeighborlessVerticesInOneCluster;
}
