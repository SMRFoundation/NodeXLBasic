    ...
    public partial class FacebookDialog : FacebookGraphDataProviderDialogBase
    {
        ...
        StartAnalysis()
        {
            ...
            try
            {
                ...
                FacebookUserModel oModel = new FacebookUserModel();

                oModel.Attributes = attributes;
                oModel.Comment = chkComment.Checked;
                oModel.DownloadFromPost = rbFromPost.Checked;
                oModel.DownloadPostsBetweenDates = rbBetween.Checked;                                
                oModel.FromDate = dtStartDate.Value;
                oModel.ToDate = dtEndDate.Value;
                oModel.FromPost = (int)nuFromPost.Value;
                oModel.ToPost = (int)nuToPost.Value;
                oModel.GetTooltips = chkTooltips.Checked;
                oModel.IncludeMe = chkIncludeMe.Checked;
                oModel.Like = chkLike.Checked;
                oModel.Limit = chkLimit.Checked;
                oModel.LimitAmount = (int)nuLimit.Value;                
                oModel.PostAuthor = chkAuthor.Checked;
                oModel.UserTagged = chkUsername.Checked;
        ...
        private string createRequiredPermissionsString()
        {
            ...
            //    }
            //}
            //if(chkMyFriendTimeline.Checked)
            //{
            //    foreach (KeyValuePair<AttributeUtils.Attribute, bool> kvp in attributes)
            //    {
            //        if (kvp.Value && attributeFriendsPermissionMapping.ContainsKey(kvp.Key.value))
            ...
            permissionsString += "read_stream,user_status,publish_actions,user_posts,";
        ...
        private void lnkFriendsNetwork_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            ShowInformation("As of April 2015 Facebook is shutting down API version 1.0, "
                            + "which means that mutual friends information is not anymore "
