...
public partial class PajekGraphAdapter : GraphAdapterBase, IGraphAdapter
{
    ...
    )
    {
        ...
        if (asFields.Length < 3 ||
            ...
            !MathUtil.TryParseCultureInvariantDouble(asFields[2],
                out dEdgeWeight)
            )
        {
            OnLoadFormatError(sLine, iLineNumber, ExpectedFormat);
        ...
        oEdgeData.Add( new PajekEdgeData(
            iFirstVertexNumber, iSecondVertexNumber, dEdgeWeight) );
    }
    ...
    )
    {
        ...
        if ( !EdgeUtil.TryGetEdgeWeight(oEdge, out dEdgeWeight) )
        {
            dEdgeWeight = DefaultEdgeWeight;
        }
