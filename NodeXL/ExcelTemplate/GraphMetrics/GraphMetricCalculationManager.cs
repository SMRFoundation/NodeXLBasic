...
/// IGraph, Microsoft.Office.Interop.Excel.Workbook)" /> to calculate the graph
/// metrics.  Call <see cref="CancelAsync" /> to stop the calculations.  Handle
/// the <see cref="GraphMetricCalculationProgressChanged" /> and <see
...
public class GraphMetricCalculationManager : Object
{
    ...
    )
    {
        ...
        if (graph == null)
        {
            // Read the workbook into a graph.  Do this from the calling thread
            // to avoid reading the Excel UI from a background thread.

            graph = ReadWorkbook(workbook, graphMetricCalculators);
        }
        ...
        oCalculateGraphMetricsAsyncArgs.Graph = graph;
    ...
    /// cref="CalculateGraphMetricsAsync(IGraph,
    ...
    /// started by <see cref="CalculateGraphMetricsAsync(IGraph,
    /// Microsoft.Office.Interop.Excel.Workbook)" />.
    /// </summary>
    ///
    /// <remarks>
    ...
    /// </param>
    ///
    /// <param name="oGraphMetricCalculators">
    /// An array of <see cref="IGraphMetricCalculator2" /> implementations, one
