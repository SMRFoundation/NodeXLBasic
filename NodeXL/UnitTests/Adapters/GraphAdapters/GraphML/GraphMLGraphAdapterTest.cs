...
public class GraphMLGraphAdapterTest : Object
{
    ...
    TestLoadGraphFromStream()
    {
        ...
        TestLoadGraphFromStreamOrFile(true);
    }
    ...
    {
        // Overall test, using sample XML from the GraphML Primer with an
        // optional graph description on the graph XML node.
        ...
        + "<graphml xmlns=\"http://graphml.graphdrawing.org/xmlns\"  "
          ...
          + "<key id=\"d1\" for=\"edge\" attr.name=\"weight\" attr.type=\"double\"/>"
          + "<graph id=\"G\" edgedefault=\"undirected\""
              + " description=\"This is the graph description.\">"
            + "<node id=\"n0\">"
              + "<data key=\"d0\">green</data>"
            + "</node>"
            + "<node id=\"n1\"/>"
        ...
        Assert.AreEqual( "This is the graph description.",
            oGraph.GetRequiredValue(ReservedMetadataKeys.GraphDescription,
                typeof(String) ) );
    }
...
+ " </graphml>"
    ...
    TestLoadGraphFromFile()
    {
        ...
        TestLoadGraphFromStreamOrFile(false);
    }
    ...
    //*************************************************************************
    //  Method: TestLoadGraphFromStreamOrFile()
    //
    /// <summary>
    /// Tests the LoadGraphFromStream() and LoadGraphFromFile() methods.
    /// </summary>
    ///
    /// <param name="bFromStream">
    /// true to test LoadGraphFromStream(), false to test LoadGraphFromFile().
    /// </param>
    //*************************************************************************

    protected void
    TestLoadGraphFromStreamOrFile
    (
        Boolean bFromStream
    )
    {
...
+ " </graphml>"
        ...
        {
            Stream oXmlStream = new StringStream(XmlString);
            oGraph = m_oGraphAdapter.LoadGraphFromStream(oXmlStream);
        }
        else
        {
            ...
            {
                oStreamWriter.Write(XmlString);
            }

            oGraph = m_oGraphAdapter.LoadGraphFromFile(m_sTempFileName);
        }
