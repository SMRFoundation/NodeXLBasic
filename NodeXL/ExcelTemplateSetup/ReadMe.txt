...
Version 3
---------
The third version of the setup program (1.0.1.120) uses the hybrid Windows/
ClickOnce technique described in "Deploying to the ClickOnce Cache" in the same
article.  A Windows Installer setup copies the template and adds the shortcut,
then calls a custom action to install the solution in the ClickOnce cache.
Because a file path is no longer embedded in the Excel template file, workbooks
created from the template can be reliably exchanged with other users.
...
Version 4
---------
The fourth version of the setup program (1.0.1.150) made a few changes to make
...
Version 5
---------
The fifth version of the setup program (1.0.1.153) eliminated the need for the
ExcelTemplateSetupStarter console application.  This was done by setting 
RequiresElevation to TRUE in ExcelTemplateSetup.vdproj.  This simplified the
setup and eliminated the "NodeXL Template Setup Starter has stopped working"
error that occurred on some machines.
